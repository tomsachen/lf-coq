...
Proof.
  ...
  - (* l = nil *)
    simpl. reflexivity.
  - (* l = x :: l' *)
  simpl. rewrite <- IHl', -> map_app_distr. simpl. reflexivity.
Qed.
...
(* If the function f is not an endomorphism on X. For example,
   if f is "division" and X is "nat", then we need Y to be "real numbers".
...
(* map f l := [x1; ... ; xn] => [f x1 ; ... ; f xn] *)

Definition fold_map {X Y: Type} (f: X -> Y) (l: list X) : list Y :=
  (* fold cons into the list l. starting element is nil list *)
...
(* for all (X Y Z:Type), (X * Y -> Z) -> X -> Y -> Z *)
Check @prod_curry.
(* for all (X Y Z:Type), (X -> Y -> Z) -> X * Y -> Z *)
Check @prod_uncurry.
...
Proof.
  intros X Y Z f []. rewrite <- uncurry_curry. reflexivity.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, advanced (nth_error_informal)
   ...
   Let X be an arbitrary type, and l a list of type (list X) of length n.
   Then, the error thrown by nth_error for nth_error X l n is None.
   In other words, the nth item of a list of length n is undefined.

   ------------------------

   Proof:

   We may assume that length l = n, by hypothesis.

   We proceed by induction on n. 

  - When (* n = 0 *), we need to show

      @nth_error X l 0 = None.

    But by the hypothesis that n = length l, we have l = nil,
    so we can evaluate the LHS of the above as

      @nth_error X [] 0 = None,

    by rewriting with nth_error.

  - When (* n = S n' *), we need to show
    ...
    Then we can evaluate the LHS of the above,
     using the fact that n \neq 0, as

         @nth_error X l pred(n)

      => @nth_error X l' n',

    which is None by the induction hypothesis.
...
Proof. simpl. unfold scc. reflexivity. Qed.
