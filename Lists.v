...
(* Print nonzeros.
 => nonzeros =
    fix nonzeros (l : natlist) : natlist := match l with
    | [ ] => [ ]
    | 0 :: t => nonzeros t
    | S h' :: t => S h' :: nonzeros t
    end. *)
...
Proof.
  ...
  - (* l = nil *)
    simpl. reflexivity.
  - (* l = n l' *)
    destruct n as [| n']; simpl; rewrite IHl'; reflexivity.
Qed.
...
(* using definition instead of fixpoint 
Definition eqblist' (l1 l2 : natlist) : bool :=
  match l1, l2 with
  | nil, nil=> true
  | _, nil | nil, _ => false
  | l1, l2 => if (included l1 l2) && (included l2 l1) then true
          else false
  end. *)
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  | nil, h :: t => false
  | h :: t, nil => false
  | h :: t, l2 => if count h l1 =? count h l2 then eqblist t (remove_one h l2)
                  else false
  end.
...
Proof.
  ...
  - (* l = nil *)
    simpl. reflexivity.
  - (* l = n :: l' *)
    simpl. rewrite eqb_refl, eqb_refl. rewrite IHl'. reflexivity.
Qed.
...
Proof.
  ...
  simpl. rewrite -> Hinv. rewrite <- Heq. rewrite <- Hinv. reflexivity.
Qed.
...
Proof.
  intros l1 l2 Heq.
  rewrite <- rev_involutive, <- Heq, -> rev_involutive. reflexivity.
Qed.
...
Proof.
  intros [| n' l'] default; reflexivity.
Qed.
