...
Definition nandb (b1:bool) (b2:bool): bool :=
  if (if b1 then b2 else false) then false
  else true.
...
Definition andb3 (b1:bool) (b2:bool) (b3:bool) : bool :=
  if (if b1 then b2 else false) then b3
  else false.
...
Proof.
  intros f H b. rewrite -> H, H. reflexivity. Qed.
...
Theorem negation_fn_applied_twice :
  ...
  forall (b : bool), f (f b) = b.
Proof.
  intros f H b. rewrite H, H, negb_involutive. reflexivity. Qed.
...
(* my original solution, using helper functions.
Definition extract_l (g : grade) : letter :=
  match g with 
  | Grade l _ => l
  end.

Definition extract_m (g : grade) : modifier :=
  match g with 
  | Grade _ m => m
  end.

Definition grade_comparison (g1 g2 : grade) : comparison :=
  match letter_comparison (extract_l g1) (extract_l g2) with 
  | Eq => match modifier_comparison (extract_m g1) (extract_m g2) with
          | Eq => Eq
          | Gt => Gt
          | Lt => Lt
          end
...
(* with some chatgpt help, for a simpler solution using pattern-matching *)
Definition grade_comparison (g1 g2 : grade) : comparison :=
  match g1, g2 with
...
Proof. Proof. simpl. reflexivity. Qed.
...
Proof. intros [] H. destruct m eqn:Em.
  { (* m = Plus*)
    - destruct l eqn:El; reflexivity. }
  {(* m = Natural *)
    - destruct l eqn:E1; reflexivity. }
  {(* m = Minus*)
    - destruct l eqn:El; reflexivity ||
...
(* using some chatgpt help, didn't quite understand the *)
Theorem lower_grade_lowers :
  forall (g : grade),
    grade_comparison (Grade F Minus) g = Lt ->
    grade_comparison (lower_grade g) g = Lt.
...
Proof.
  ...
  rewrite -> apply_late_policy_unfold, H. reflexivity.
Qed.
...
Proof.
  ...
  rewrite -> apply_late_policy_unfold. rewrite -> H1, H2.
  destruct g, m; reflexivity.
Qed.
